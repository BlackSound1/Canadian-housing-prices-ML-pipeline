...
def transform_custom(merged_df, *args, **kwargs):
    ...
    rmse1 = round(mean_squared_error(y_val, ridge_predict, squared=False), 2)
    
    print(f"The Ridge model has an RMSE of: {rmse1} with the VALIDATION data")

    # Make predictions on the TESTING data
    ridge_predict = best_ridge_model.predict(X_test)
    rmse2 = round(mean_squared_error(y_test, ridge_predict, squared=False), 2)

    print(f"The Ridge model has an RMSE of: {rmse2} with the TESTING data")
    ...
    pred_val_2020 = round(list(best_ridge_model.predict(test_df_2020))[0], 2)
    ...
    pred_val_2021 = round(list(best_ridge_model.predict(test_df_2021))[0], 2)
    ...
    pred_val_2022 = round(list(best_ridge_model.predict(test_df_2022))[0], 2)
    print(f"Non-cleaned predicted value for 2022-09: {pred_val_2022}. Actual value for 2022-09: 639485.32")

    rmse_data = {'rmse_val': rmse1, 'rmse_test': rmse2}
